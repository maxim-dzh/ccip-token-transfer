...
task(
  ...
  .addParam(`receiverNetwork`, `CrossChainReceiver contract network`)
  .addOptionalParam(
    `transferUsdc`,
  ...
  .setAction(
    async (taskArguments: TaskArguments, hre: HardhatRuntimeEnvironment) => {
      ...
      const crossChainReceiverAddress = getDeploymentInfo(
        `${taskArguments.receiverNetwork}-CrossChainReceiver`
      ).crossChainReceiver;
      ...
      console.log(
        `ℹ️  Attempting to call the transferUsdc function on the TransferUSDC smart contract on the ${networkName} blockchain with amount ${taskArguments.amount}. We send to ${destinationChainSelector} ${taskArguments.receiverNetwork}`
      );
      spinner.start();
      const tx = await transferUSDC.transferUsdc(
        ...
        // the gas limit that we estimated via hardhat tests
        62810
      );
      await tx.wait();
